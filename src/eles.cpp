        ...
        for(j=0;j<n_fpts_per_ele;j++) {
            ...
            if(viscous) {
	  		...
	  		if(n_dims==2)
	  		{
                ...
                tnorm_dot_inv_detjac_mul_jac(0)=(tnorm_fpts(0,j)*ys)-(tnorm_fpts(1,j)*yr);
                tnorm_dot_inv_detjac_mul_jac(1)=-(tnorm_fpts(0,j)*xs)+(tnorm_fpts(1,j)*xr);
...
void eles::initialize_grid_vel(void)
{
     ...
     * n_spts_per_ele = (potentially) variable */
...
void eles::set_transforms_inters_cubpts(void)
{
  ...
  for (int i=0;i<n_inters_per_ele;i++)
  {
    ...
    for (int l=0;l<n_inters_per_ele;l++)
    {
        ...
        if (motion) {
            calc_d_pos_dyn(loc,bdy_ele2ele(i),d_pos);
        }else{
  ...
  for (i=0; i<n_eles; i++) {
    ...
    for (j=0; j<n_upts_per_ele; j++) {
        if (in_norm_type == 1) {
          ...
          cell_sum += pow(div_tconf_upts(0)(j, i, in_field)/detjac_upts(j,i),2);
        }
    }
    sum += cell_sum;
  }
